...
function AllStudents() {
  const getCSV = async () => {
    try {
      ...
      if (data.success && data.csv) {
        ...
        toast.error(data.errors[0]?.msg || 'Failed to generate CSV', {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const getAll = async () => {
    ...
    // for the access the student number in home page
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-5 items-center justify-center">
      ...
      <div className="w-96 flex justify-center">
        {/* <button
          onClick={getCSV}
          className="px-20 py-3 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl shadow-custom-black"
        >
          Download List
        </button> */}
        <ToastContainer
        ...
        <ul role="list" className="divide-y divide-gray-700 text-white">
          {allStudents.length === 0 ? (
            ...
            allStudents.map((student) => (
              <li className="my-2 py-3 px-5 rounded sm:py-2 hover:bg-highlight hover:scale-105 transition-all" key={student._id}>
