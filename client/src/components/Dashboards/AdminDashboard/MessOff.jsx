...
function MessOff() {
  ...
  const updateRequest = async (id, status) => {
    const res = await fetch("https://hostel-management-ofhb.vercel.app/api/messoff/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const reject = (id) => {
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-4 items-center justify-center">
      ...
      <div className="bg-secondary px-10 py-5 rounded-xl shadow-custom-black sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        ...
        <ul role="list" className="divide-y divide-gray-700 text-white">
          {newReqs.length === 0 ? (
            ...
            newReqs.map((req) => (
              <li
                className="my-2 group py-3 px-5 rounded sm:py-2 hover:bg-highlight hover:shadow-xl hover:scale-105 transition-all cursor-pointer"
                key={req.id}
              >
